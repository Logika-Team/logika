...
use Logika\Core\ImageOverrides;
...
if ( ! $field ) {
	...
	$valid = ImageOverrides::validateValue( true, $image_id, $field, 'acf[field_home_hero_boy_image_override]' );
	if ( true !== $valid ) {
		$errors[] = 'A valid hero override is rejected.';
	}

	wp_update_attachment_metadata( (int) $image_id, array( 'width' => 225, 'height' => 440 ) );
	$invalid = ImageOverrides::validateValue( true, $image_id, $field, 'acf[field_home_hero_boy_image_override]' );
	if ( true === $invalid ) {
		$errors[] = 'An override with an invalid aspect ratio is accepted.';
	}

	wp_update_post( array( 'ID' => $image_id, 'post_mime_type' => 'image/gif' ) );
	$unsupported = ImageOverrides::validateValue( true, $image_id, $field, 'acf[field_home_hero_boy_image_override]' );
	if ( true === $unsupported ) {
