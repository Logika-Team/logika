...
if ( ! str_contains( $single, 'Logika_Theme_Article_Page::render' ) || ! class_exists( 'Logika_Theme_Article_Page' ) || ! str_contains( $fields, 'article_cover_image' ) || ! str_contains( $fields, 'article_related_posts' ) || ! str_contains( $fields, 'article_faq_items' ) || ! function_exists( 'acf_get_field' ) ) {
	fwrite( STDERR, "Posts do not use the dynamic article template and editorial fields.\n" );
	exit( 1 );
}

foreach ( array( 'article_cover_image', 'article_related_posts', 'article_faq_items', 'media_center_topics' ) as $field ) {
